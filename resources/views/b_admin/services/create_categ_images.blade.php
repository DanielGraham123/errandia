...
@section('section')
    <div class="container">
        ...
        <form method="POST" action="{{ route('business_admin.products.create_update', $shop->slug) }}" enctype="multipart/form-data">
            @csrf
                ...
                <div class="d-flex flex-wrap my-3 border-left border-right rounded">
                    @foreach ($categories as $cat)
                        <span class="d-inlineblock rounded border bg-light py-1 px-3 my-2 mx-2">
                            <input type="checkbox" class="input mx-2 disabled" name="categories[]" value="{{ $cat->id }}">
